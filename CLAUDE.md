...
`_data/termine.yml` is normally regenerated by CI before each build (see below) and is gitignored — it never gets committed, so it's safe to fetch real data locally for design work. To regenerate it locally:

```bash
KONZERTMEISTER_API_KEY="dein-key" bundle exec ruby script/fetch_termine.rb
```

(or rely on `.env`, see below). Without a key set, the script exits 0 and leaves `_data/termine.yml` as-is (or simply absent, on a fresh checkout) — local builds work fine either way, the include just renders its "keine Termine" fallback.
...
- **`_data/termine.yml`** — upcoming event data, machine-generated (see below) and gitignored (never committed). Never hand-edit; it gets overwritten on the next build/fetch, and may simply not exist on a fresh checkout.
